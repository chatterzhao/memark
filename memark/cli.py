...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="memark", description="Promote room packages into Graphify-ready corpus.")
    ...
    promote_parser = subparsers.add_parser("promote", help="Promote room package JSON into Graphify corpus")
    promote_parser.add_argument("input", nargs="?", default="inbox", help="JSON file or directory to consume")
    ...
    run_parser = subparsers.add_parser("run", help="Promote room packages and then run Graphify")
    ...
    run_parser.add_argument("--input", default="inbox", help="JSON file or directory to consume")
...
def cmd_promote(args: argparse.Namespace) -> int:
    ...
    if not input_path.is_absolute():
        input_path = (config.workspace / input_path).resolve()
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    inbox_files = [item for item in config.inbox_dir.iterdir() if item.is_file()]
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    project = slugify(args.project or config.default_project)
    graphify_bin = args.graphify_bin or config.graphify_bin
    command = [graphify_bin, str(config.corpus_project_dir(project))]
    if args.update:
        command.append("--update")
    if args.wiki:
        command.append("--wiki")
