...
再进一步，当前项目的成功标准不能只是“把两者串起来”。

必须满足：

- 用了 `MemArk` 之后，历史找回不能比单用 `MemPalace` 更差
- 用了 `MemArk` 之后，结构理解不能比单用 `Graphify` 更差
- 用了 `MemArk` 之后，自动喂数据、自动加工、自动消费三段都不能比单用上游更差
- 用了 `MemArk` 之后，AI 和人类管理者的项目续接效果必须更好，而不是更绕、更慢、更多噪音

如果 `MemArk` 只是多加一层包装，却让消费质量下降、步骤变多或边界更混乱，那么这个项目就算失败。
...
下一阶段真正应该补的，不是继续堆零散命令，而是把下面五层收成默认体验：
