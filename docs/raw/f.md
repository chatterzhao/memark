...
- **增量同步**：只处理新增对话，最小化开销
- **按需下钻**：知识图谱中的每一条结论都保留引用 ID，可一键回溯原始对话
...
**收益**：最终知识库不是冰冷的原文摘录，而是**经过人类智慧 + AI 推理加热过的、可直接使用的洞察**。
...
| `memark serve` | 启动 MemPalace MCP 服务器 |
...
知识图谱页面中的 `[^ref_room123]` 是保留的引用 ID。Obsidian 可配置自定义命令：
...
- **MemPalace** 负责完整的记忆存储、房间抽象、摘要生成（AAAK）和 MCP 访问
- **MemArk** 只通过其 MCP 接口获取“房间摘要”，不直接操作底层数据库
- 用户可单独使用 MemPalace 做记忆检索，也可叠加 MemArk 获得知识编译能力
